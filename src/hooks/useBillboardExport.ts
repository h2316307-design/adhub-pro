...
export const useBillboardExport = () => {
  ...
  const exportAvailableAndUpcoming = async (billboards: any[], isContractExpired: (endDate: string | null) => boolean) => {
    try {
      ...
      const filteredBillboards = billboards.filter((billboard: any) =>
        isAvailableOrUpcomingForExport(billboard, threeMonthsFromNow, isContractExpired)
      );
  ...
  const exportAvailableAndUpcomingWithContracts = async (
    ...
    try {
      ...
      const filteredBillboards = billboards.filter((billboard: any) => {
        ...
        return isAvailableOrUpcomingForExport(billboard, threeMonthsFromNow, isContractExpired);
      });
  ...
  const uploadFollowUpToSite = async (billboards: any[]) => {
    try {
      ...
      const wbOut = XLSX.write(wb, { type: 'base64', bookType: 'xlsx' });
      const { uploadFileToGoogleDrive } = await import('@/services/imageUploadService');
      const url = await uploadFileToGoogleDrive(wbOut, 'follow-up.xlsx', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'billboard-exports', true);
  ...
  const uploadAllToSite = async (billboards: any[]) => {
    try {
      ...
      const wbOut = XLSX.write(wb, { type: 'base64', bookType: 'xlsx' });
      const { uploadFileToGoogleDrive } = await import('@/services/imageUploadService');
      const url = await uploadFileToGoogleDrive(wbOut, 'all-billboards.xlsx', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'billboard-exports', true);
