...
export const useBillboardData = () => {
  ...
  const updateBillboardVisibilityLocal = useCallback((billboardId: string | number, isVisibleInAvailable: boolean) => {
    setBillboards(prev => prev.map((b: any) => {
      ...
      if (currentId !== String(billboardId)) return b;

      const statusValue = String(b.Status ?? b.status ?? '').trim().toLowerCase();
      ...
      const shouldResetBlockingMaintenance = isVisibleInAvailable && !hasContract &&
      ...
      return {
        ...b,
        is_visible_in_available: isVisibleInAvailable,
        ...(shouldResetBlockingMaintenance ? { maintenance_status: 'operational', maintenance_type: null } : {})
      };
    }));
  }, []);
