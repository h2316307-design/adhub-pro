...
export const useContractCalculations = ({
  ...
  const rentalCostOnly = useMemo(() => Math.max(0, totalAfterDiscount - actualInstallationCost), [totalAfterDiscount, actualInstallationCost]);
  ...
  // When installation is disabled, totalAfterDiscount = rentalCostOnly, so calculate from totalAfterDiscount
  const operatingFee = useMemo(() => {
    const baseForFee = !installationEnabled ? totalAfterDiscount : rentalCostOnly;
