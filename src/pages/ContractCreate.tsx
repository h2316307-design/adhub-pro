...
export default function ContractCreate() {
  ...
  // ✅ Load billboards with availability filter — CORRECTED
  useEffect(() => {
    (async () => {
      try {
        ...
        console.log('✅ تم تحميل', data?.length || 0, 'لوحة من قاعدة البيانات');
        if (data && data.length > 0) {
          const withCoords = data.filter(b => b.GPS_Coordinates).length;
          console.log('📍 عدد اللوحات التي لديها إحداثيات GPS:', withCoords, 'من أصل', data.length);
        }
              ...
              <TabsContent value="map" className="mt-0">
                <div className="relative w-full" style={{ height: '700px' }}>
                  <BillboardSelectionMap
                    ...
                    onToggleSelect={(billboard) => toggleSelect(billboard)}
                    onSelectAll={() => {
