...
export default function Users() {
  ...
  const fetchPage = async (pageIndex: number) => {
    ...
    // Fallback: جلب البيانات من profiles مع الصلاحيات
    let resp = await supabase
      .from('profiles')
      .select('id, name, email, phone, company, created_at, approved, status', { count: 'exact' })
                  ...
                  {rows.map((r) => (
                    <TableRow key={r.id}>
                      ...
                      <TableCell>
                        ...
                        {r.status === 'approved' && <Badge className="bg-green-600">موافق عليه</Badge>}
                      ...
                      <TableCell>
                        <div className="flex gap-2 flex-wrap">
                          {r.status === 'pending' && (
                            <>
                              <Button size="sm" onClick={() => handleApproveUser(r.id, 'approved')} className="bg-green-600 hover:bg-green-700">قبول</Button>
                              <Button size="sm" variant="destructive" onClick={() => handleApproveUser(r.id, 'rejected')}>رفض</Button>
                            </>
                          )}
                          {r.status === 'approved' && (
                            <>
                              <Button size="sm" onClick={() => handleSave(r)} disabled={savingId === r.id}>حفظ</Button>
                              <Button size="sm" variant="outline" onClick={() => handleOpenPermissions(r.id)}>الصلاحيات</Button>
                              {isAdmin && (
                                <Button size="sm" variant="outline" onClick={() => { setPasswordTargetId(r.id); setPasswordNew(''); setPasswordConfirm(''); setPasswordModalOpen(true); }}>كلمة المرور</Button>
                              )}
                            </>
                          )}
                        </div>
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={async () => {
                ...
                try {
                  ...
                  const resp = await fetch(`${supabaseUrl}/functions/v1/admin-set-password`, {
                    method: 'POST',
                    headers: {
                ...
                <div key={page.value} className="flex items-center gap-2">
                  <Switch
                    ...
                    onCheckedChange={(checked) => {
                      if (checked) {
                        setSelectedPermissions([...selectedPermissions, page.value]);
                      } else {
                        setSelectedPermissions(selectedPermissions.filter(p => p !== page.value));
                      }
                    }}
                  />
