                        ...
                        {(() => {
                          ...
                          return (
                            ...
                            >
                              ...
                              {allSelected ? 'إلغاء التحديد' : `تحديد الكل (${allTeamTaskIds.length})`}
                            </Button>
                          );
                        })()}
                      ...
                      {teamTasks.map(task => {
                        ...
                        return (
                          <MobileTaskCard
                            ...
                            onAddBillboards={() => {
                              const existingIds = taskItems.map(i => i.billboard_id);
                              setSelectedTaskForAddBillboards({
                                taskId: task.id,
                                contractId: task.contract_id,
                                contractIds: taskContractIds,
                                existingBillboardIds: existingIds
                              });
          ...
          >
            <Card className="bg-gradient-to-r from-primary to-primary/80 text-white px-6 py-4 shadow-2xl rounded-2xl border-0">
              <div className="flex items-center gap-4 flex-wrap justify-center">
                ...
                {/* زر تحديد كل لوحات المهمة للفرقة الحالية */}
                {(() => {
                  // البحث عن المهمة الحالية من أول لوحة محددة
                  ...
                  // جلب كل اللوحات في نفس المهمة فقط (نفس الفرقة ونفس العقد)
                  const taskItems = allTaskItems.filter(item => {
                    return item.task_id === currentTask.id && item.status !== 'completed';
                  });
                  
                  const allSelected = taskItems.every(item => selectedItemsForDate.includes(item.id));
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (allSelected) {
                          ...
                          setSelectedItemsForDate(taskItems.map(item => item.id));
                        }
                      }}
                    >
                      {allSelected ? (
                        ...
                        <>
                          ...
                          تحديد كل المهمة ({taskItems.length})
                        </>
                      )}
                    </Button>
                  );
