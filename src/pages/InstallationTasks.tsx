...
export default function InstallationTasks() {
  ...
  // ✅ استخراج العقود الفعلية لكل مهمة - نعتمد على contract_ids المخزنة في المهمة نفسها
  // ولا نستخرج من اللوحات لأن اللوحة قد تكون انتقلت لعقد جديد فيظهر خطأ "مهمة مدمجة"
  const derivedContractIdsByTaskId = useMemo(() => {
    const map = new Map<string, number[]>();

    tasks.forEach((task: any) => {
      // إذا المهمة تحتوي على contract_ids مخزنة (مهمة مدمجة فعلاً)، نستخدمها
      if (task.contract_ids && Array.isArray(task.contract_ids) && task.contract_ids.length > 1) {
        map.set(task.id, [...task.contract_ids].sort((a: number, b: number) => a - b));
      } else {
        // غير ذلك، نستخدم contract_id الواحد فقط
        map.set(task.id, [task.contract_id].filter(Boolean));
      }
    });
