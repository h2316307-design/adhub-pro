...
export default function Billboards() {
      ...
      if (billboardId && billboards.length > 0) {
        ...
        if (billboard) {
          ...
          try {
            ...
            // ✅ Refetch to guarantee sync with server state
  ...
  const filteredBillboards = useMemo(() => {
    ...
    const afterFilters = searched.filter((billboard) => {
      ...
      const isAvailable = (!hasContract) || (hasContract && contractExpired);
    ...
    return afterFilters;
  }, [billboards, searchQuery, selectedStatuses, selectedCities, selectedSizes, selectedMunicipalities, selectedDistricts, selectedAdTypes, selectedCustomers, selectedContractNumbers, isContractExpired, isMarketer, marketerCustomerId, marketerBillboardIds]);
  ...
  const availableBillboardsCount = useMemo(() => {
    return billboards.filter((billboard: any) => {
      // ✅ استبعاد لوحات الشركات الصديقة المخفية
      if (billboard.friend_company_id && billboard.is_visible_in_available === false) return false;

      // ✅ استبعاد اللوحات المخفية يدوياً
      ...
      // استبعاد لوحات الإزالة
      if (statusValue === 'إزالة' || statusLower === 'ازالة' || maintenanceStatus === 'تحتاج ازالة لغرض التطوير' || maintenanceStatus === 'لم يتم التركيب' || maintenanceStatus === 'تمت الإزالة') return false;
