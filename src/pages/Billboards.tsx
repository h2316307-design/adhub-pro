...
export default function Billboards() {
      ...
      if (billboardId && billboards.length > 0) {
        ...
        if (billboard) {
          ...
          try {
            ...
            if (newValue && !hasContract && (statusValue === 'متاح' || statusValue === 'available' || statusValue === '')) {
  ...
  const toggleSelectedVisibility = async (billboardIds: number[], visible: boolean) => {
    ...
    for (const id of billboardIds) {
      const billboard = billboards.find((b: any) => Number(b.ID || b.id) === Number(id)) as any;
      const updatePayload: any = { is_visible_in_available: visible };

      if (visible && billboard) {
