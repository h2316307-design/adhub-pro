...
export default function ContractEdit() {
  ...
  const { canEdit: canEditFn, isAdmin } = useAuth();

  // التحقق من صلاحية التعديل - إذا لم يكن لديه صلاحية يتم إعادة توجيهه
  useEffect(() => {
    if (!canEditFn('contracts') && !isAdmin) {
      navigate('/admin/contracts', { replace: true });
    }
  }, [canEditFn, isAdmin, navigate]);
      ...
      try {
        ...
        if (s && e) {
          ...
          if (!isNaN(sd.getTime()) && !isNaN(ed.getTime())) {
            ...
            const isDayBasedDuration = savedDuration.includes('يوم') || (diffDays % 30 !== 0);
