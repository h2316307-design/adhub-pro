...
export default function ContractEdit() {
    ...
    (async () => {
      ...
      try {
        ...
        if (baseRentFromDB > 0) {
          ...
          setOriginalTotal(savedCustomerTotal); // ✅ FIX: Use Total (what customer pays), NOT Total Rent
          // ✅ When base_rent is saved, we DON'T use stored prices (we use the fresh calculation)
          setUseStoredPrices(false);
          console.log('✅ Loaded base_rent from DB:', baseRentFromDB);
          console.log('✅ Loaded originalTotal (customer total) from DB:', savedCustomerTotal);
        } else {
          // ❌ No base_rent saved - use Total temporarily
          const savedTotal = savedCustomerTotal || Number(c['Total Rent'] || 0);
          setRentCost(savedTotal);
          setOriginalTotal(savedTotal || 0);
          
          // Use stored prices if we have them
          const hasSavedPrices = savedTotal > 0 || c.billboard_prices;
          setUseStoredPrices(hasSavedPrices);
          console.log('⚠️ No base_rent in DB, price source:', hasSavedPrices ? 'Stored prices' : 'Fresh prices');
        }
  ...
  const distributeWithInterval = React.useCallback((config: {
    ...
    for (let i = 0; i < numberOfRecurringPayments; i++) {
      ...
      // ✅ Business rule: always keep 1st = عند التوقيع, 2nd = عند التركيب
      const overallIndex = newInstallments.length; // index in the final array before push
      const paymentTypeForThis = overallIndex === 0
        ? 'عند التوقيع'
        : (overallIndex === 1 ? 'عند التركيب' : intervalLabel);

      const dueDateIso = overallIndex === 0
        ? firstDate
        : (overallIndex === 1 ? calculateDueDate('عند التركيب', 1, firstDate) : dueDate.toISOString().split('T')[0]);
