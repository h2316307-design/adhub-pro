...
import { Loader2, DollarSign, Plus, Calculator, TrendingUp, TrendingDown, Lock, Calendar, Hash, Printer, Edit, Trash2 } from 'lucide-react';
...
interface Contract {
  ...
  feePercent: number;
  feeAmount: number;
  fullFeeAmount: number; // القيمة الكاملة للنسبة
  collectedFeeAmount: number; // النسبة المتحصلة فعلياً
  rent_cost: number; // سعر الإيجار فقط
  installation_cost: number; // تكلفة التركيب
  print_cost: number; // تكلفة الطباعة
  total_amount: number; // الإجمالي الكلي (rent + installation + print)
  total_paid: number;
  collectionPercentage: number; // نسبة التحصيل من الإجمالي الكلي
  start_date: string;
  status: string;
}
...
const normalizeContract = (record: any): Contract => {
  ...
  // ✅ حساب الإجمالي الكلي (Total)
  const totalAmount = rentCost + installationCost + printCost;
  
  // ✅ حساب المبلغ المدفوع ونسبة التحصيل من الإجمالي الكلي
  ...
  // ✅ حساب القيمة الكاملة للنسبة (من الإجمالي الكلي)
  const fullFeeAmount = Math.round(totalAmount * (normalizedFeePercent / 100));
  
  // ✅ حساب النسبة المتحصلة فعلياً (بناءً على المدفوعات)
  const collectedFeeAmount = Math.round(totalPaid * (normalizedFeePercent / 100));
  
  const feeAmount = feeAmountRaw > 0 ? feeAmountRaw : collectedFeeAmount;

  // ✅ إصلاح مشكلة duplicate keys
...
export default function OperatingExpenses() {
  ...
  const loadData = async () => {
    try {
      ...
      if (contractsError) {
        ...
        let mappedContracts = (contractsData || []).map(normalizeContract);
        ...
        try {
          ...
          mappedContracts = mappedContracts.map((c) => {
            ...
            const collectedFee = Math.round(paid * (c.feePercent / 100));
            return {
              ...c,
              total_paid: paid,
  ...
  return (
    <div className="expenses-container">
      ...
      <Card>
        <CardHeader>
          <CardTitle>مصروفات التشغيل - العقود وحالة الحسبة</CardTitle>
        </CardHeader>
        <CardContent>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">التركيب والطباعة</TableHead>
              ...
              <TableBody>
                {contracts.map(contract => {
                  ...
                  const installAndPrintCost = contract.installation_cost + contract.print_cost;
                  const formattedInstallPrint = installAndPrintCost.toLocaleString('ar-LY', { maximumFractionDigits: 2 });
                  ...
                  return (
                    <TableRow key={id}>
                      ...
                      <TableCell className="text-right">
                        <Badge variant={contract.collectionPercentage >= 100 ? "default" : contract.collectionPercentage >= 50 ? "secondary" : "outline"}>
                          {formattedCollectionPct}%
                        </Badge>
                      </TableCell>
                      <TableCell className="text-right text-muted-foreground">{formattedFullFee} د.ل</TableCell>
                      <TableCell className={`text-right font-semibold ${excluded || closed ? 'expenses-amount-excluded' : 'expenses-amount-calculated'}`}>
      ...
      {selectedWithdrawalForReceipt && (
        <ExpenseReceiptPrintDialog
          ...
          expense={{
            ...selectedWithdrawalForReceipt,
            description: 'سحب من مصروفات التشغيل',
