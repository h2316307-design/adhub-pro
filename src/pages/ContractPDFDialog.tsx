...
function buildContractPdfTitle(params: {
  ...
  const { contractNumber, yearlyCode, adType, customerName, billboards, currencySymbol, totalAmount, isOffer } = params;
...
export default function ContractPDFDialog({ open, onOpenChange, contract }: ContractPDFDialogProps) {
  ...
  const handlePrintInvoice = async () => {
    ...
    try {
      ...
      showPrintPreview(html, invoiceTitle);
...
<body>
  ...
  const handleDownloadContractPDF = async () => {
    ...
    try {
      ...
      try {
        ...
        const pdfFileName = `عقد_${contractNumber}_${customerData.name}.pdf`;
  ...
  const handleSendContractWhatsApp = async () => {
    ...
    try {
      ...
      const contractNumber = contract?.Contract_Number || contract?.id || '';
      const pdfFileName = `عقد_${contractNumber}_${customerData.name}.pdf`;
      const pdfUrl = await uploadFileToGoogleDrive(base64Data, pdfFileName, 'application/pdf', 'contracts');
