...
import { Edit, Trash2, Printer, Plus, AlertCircle, TrendingUp } from 'lucide-react';
    ...
    <TableRow className={rowClassName}>
      <TableCell className="font-medium">
          ...
          <div className="flex gap-1">
            {customer.isCustomer && (
              <Badge variant="outline" className="bg-accent/10 text-accent-foreground border-accent/30 text-xs">
                زبون
              </Badge>
            )}
            {customer.isSupplier && (
              <Badge className="bg-gradient-primary text-primary-foreground text-xs">
                مورد
                {customer.supplierType === 'billboard_rental' && ' (إيجار لوحات)'}
                {customer.supplierType === 'general_purchases' && ' (مشتريات عامة)'}
      ...
      <TableCell className="text-right">{customer.contractsCount}</TableCell>
      <TableCell className="text-right font-semibold">{customer.totalRent.toLocaleString('ar-LY')} د.ل</TableCell>
      <TableCell className="text-right text-green">{customer.totalPaid.toLocaleString('ar-LY')} د.ل</TableCell>
      <TableCell className="text-right text-blue font-semibold">{customer.accountBalance.toLocaleString('ar-LY')} د.ل</TableCell>
      <TableCell className="text-right text-red font-semibold">{remaining.toLocaleString('ar-LY')} د.ل</TableCell>
      <TableCell>
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2">
            <Progress value={paymentPercentage} className="h-2 flex-1" />
            <span className="text-xs font-medium min-w-[40px] text-right">{paymentPercentage}%</span>
          </div>
        </div>
      </TableCell>
      <TableCell>
        {isOverdue ? (
          ...
          <div className="flex flex-col gap-1">
            <Badge variant="destructive" className="text-xs bg-orange">
              دين غير مسدد
            </Badge>
            <div className="text-xs text-muted-foreground">
          ...
          <Badge variant="outline" className="text-xs bg-accent/10 text-green border-accent/30">
      ...
      <TableCell>
        <div className="flex gap-2 flex-wrap">
          <Button size="sm" onClick={onViewBilling}>عرض فواتير</Button>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-primary to-primary/60 bg-clip-text text-transparent">إدارة الزبائن</h1>
          <p className="text-muted-foreground mt-1">إدارة شاملة لبيانات وحسابات الزبائن</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          <Button onClick={() => navigate('/admin/overdue-payments')} variant="destructive" size="lg" className="gap-2">
          ...
          <Button onClick={() => { setEditingCustomerId(null); setNewCustomerOpen(true); setCustomerNameInput(''); setCustomerPhoneInput(''); setCustomerCompanyInput(''); setIsCustomerChecked(true); setIsSupplierChecked(false); setSupplierTypeInput(''); setSelectedPrinterId(''); }} size="lg">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="border-primary/20 bg-gradient-to-br from-primary/5 to-transparent">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-primary">{customersSummary.length}</div>
          </CardContent>
        </Card>
        ...
        <Card className="border-accent/20 bg-gradient-to-br from-accent/5 to-transparent">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-green">{customersSummary.reduce((s, c) => s + c.totalRent, 0).toLocaleString('ar-LY')} <span className="text-lg">د.ل</span></div>
          </CardContent>
        </Card>
        ...
        <Card className="border-primary/20 bg-gradient-to-br from-primary/5 to-transparent">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-blue">{totalAllPaid.toLocaleString('ar-LY')} <span className="text-lg">د.ل</span></div>
          </CardContent>
        </Card>
        ...
        <Card className="border-destructive/20 bg-gradient-to-br from-destructive/5 to-transparent">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-red">{customersSummary.reduce((s, c) => s + (c.totalRent - c.totalPaid), 0).toLocaleString('ar-LY')} <span className="text-lg">د.ل</span></div>
          </CardContent>
        </Card>
      </div>
