        ...
        actions={
          <>
            <ToggleGroup type="single" value={viewMode} onValueChange={(v) => v && setViewMode(v as 'cards' | 'table')} className="rounded-full bg-muted/40 p-0.5">
              <ToggleGroupItem value="cards" aria-label="عرض الكروت" className="gap-2 rounded-full data-[state=on]:bg-background data-[state=on]:shadow-sm">
                <LayoutGrid className="h-4 w-4" />
                <span className="hidden sm:inline">كروت</span>
              </ToggleGroupItem>
              <ToggleGroupItem value="table" aria-label="عرض الجدول" className="gap-2 rounded-full data-[state=on]:bg-background data-[state=on]:shadow-sm">
                <List className="h-4 w-4" />
                <span className="hidden sm:inline">جدول</span>
              </ToggleGroupItem>
            </ToggleGroup>
            <Button variant="outline" onClick={() => setAlertsDialogOpen(true)} className="gap-2 rounded-full">
              <Send className="h-4 w-4" />
              <span className="hidden sm:inline">تنبيهات</span>
            </Button>
            <Button variant="outline" onClick={() => setReportDialogOpen(true)} className="gap-2 rounded-full">
              <FileText className="h-4 w-4" />
              <span className="hidden sm:inline">تقرير</span>
            </Button>
            {canCreateOrDelete && (
              <Button
                className="gap-2 rounded-full bg-gradient-to-l from-primary to-primary/80 hover:shadow-[var(--shadow-luxury)] transition-shadow"
