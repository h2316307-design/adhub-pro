...
import { Plus, Eye, Edit, Trash2, Calendar, User, DollarSign, Search, Filter, Building, AlertCircle, Clock, CheckCircle, Printer, RefreshCcw, Hammer, Wrench, Percent, PaintBucket, FileText } from 'lucide-react';
...
export default function Contracts() {
  ...
  useEffect(() => {
    const handleBillboardPrint = (event: any) => {
      const { contractNumber, billboards, designFaceA, designFaceB } = event.detail;
      setBillboardPrintData({ contractNumber, billboards, designFaceA, designFaceB });
  ...
  return (
    <div className="space-y-6" dir="rtl">
                          ...
                          <Button
                            ...
                            onClick={async () => {
                              try {
                                ...
                                const designA = (contractWithBillboards as any).design_face_a_path || null;
                                const designB = (contractWithBillboards as any).design_face_b_path || null;

                                if (billboardsData.length === 0) {
                                  toast.warning('لا توجد لوحات لهذا العقد');
                                  return;
                                }
      ...
      {billboardPrintData && (
        <Dialog open={billboardPrintOpen} onOpenChange={setBillboardPrintOpen}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>طباعة اللوحات المنفصلة</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">
                سيتم طباعة كل لوحة في صفحة منفصلة مع جميع تفاصيلها
              </p>
              ...
              <div className="flex items-center gap-2 p-3 bg-muted rounded-lg">
                ...
                <Label htmlFor="include-designs" className="cursor-pointer">
                  تضمين التصاميم (الوجه الأمامي والخلفي)
                </Label>
              </div>

              {(!billboardPrintData.designFaceA && !billboardPrintData.designFaceB) && (
                <div className="text-sm text-orange-600 bg-orange-50 p-3 rounded">
                  ⚠️ لم يتم إضافة روابط التصاميم لهذا العقد
                </div>
              )}

              <div className="text-sm text-muted-foreground bg-muted/50 p-3 rounded">
                <strong>سيتم طباعة:</strong> {billboardPrintData.billboards.length} لوحة ({billboardPrintData.billboards.length} صفحة)
              </div>

              <div className="flex gap-2 pt-4">
                <Button
                  onClick={() => {
                    setBillboardPrintOpen(false);
                    setTimeout(() => {
                      const printBtn = document.querySelector('[data-billboard-print-trigger]') as HTMLButtonElement;
                ...
                <Button variant="outline" onClick={() => setBillboardPrintOpen(false)} className="flex-1">
                  إلغاء
                </Button>
              </div>
            </div>
      ...
      {billboardPrintData && (
        <div style={{ display: 'none' }}>
          <BillboardPrintIndividual
            ...
            designFaceA={includeDesigns ? billboardPrintData.designFaceA : null}
            designFaceB={includeDesigns ? billboardPrintData.designFaceB : null}
          />
        </div>
      )}
    </div>
