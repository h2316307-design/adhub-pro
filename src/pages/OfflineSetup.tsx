...
`;
  ...
  const handleDownloadImages = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < imageUrls.length; i++) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            urlMappings.push({ original: url, local: `/images/${filename}` });
          }
        } catch (e) {
      ...
      let updateSQL = `-- Update image paths for local use\n\n`;
      for (const mapping of urlMappings) {
        updateSQL += `-- Original: ${mapping.original}\n`;
        updateSQL += `-- Local: ${mapping.local}\n\n`;
      }
      zip.file('update_image_paths.sql', updateSQL);
