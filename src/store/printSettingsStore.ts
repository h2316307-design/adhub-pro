  ...
  const fetchSettings = useCallback(async () => {
    ...
    try {
      ...
      if (!newError && newSettings && newSettings.length > 0) {
        ...
        // تحديث الإعدادات المشتركة من أول سجل
        if (newSettings.length > 0) {
          ...
          dispatch({
            ...
            payload: {
              ...
              company_subtitle: first.company_subtitle ?? '',
              company_address: first.company_address ?? '',
              company_phone: first.company_phone ?? '',
              ...
              logo_path: first.logo_path ?? '/logofaresgold.svg',
              logo_size: first.logo_size ?? 60,
              logo_position: (first.logo_position ?? 'right') as AlignmentType,
