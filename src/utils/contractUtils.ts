...
export const isBillboardExplicitlyMarkedAvailable = (billboard: any): boolean => {
  ...
  return (
    ...
    !contractNumber &&
    (status === 'available' || status === 'متاح' || status === '')
  );
};
...
export const isBillboardBlockedFromAvailability = (billboard: any): boolean => {
  ...
  // إذا قام المستخدم بإظهار لوحة متاحة وغير مؤجرة يدوياً، لا نحجبها بسبب بقايا حالة صيانة قديمة
  // مثل maintenance_status = removed. أما Status = إزالة فيبقى مستبعداً دائماً.
  if (isBillboardExplicitlyMarkedAvailable(billboard)) {
    return status === 'إزالة' || status === 'ازالة' || status === 'removed';
  }
