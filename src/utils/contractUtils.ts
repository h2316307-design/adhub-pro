...
export const isBillboardAvailable = (billboard: any): boolean => {
  const contractNumber = billboard.Contract_Number || billboard.contractNumber;
  const endDate = billboard.Rent_End_Date || billboard.rent_end_date || billboard.contract?.end_date;
  const status = (billboard.Status || billboard.status || '').toString().trim();
  const maintenanceStatus = billboard.maintenance_status;
  const maintenanceType = billboard.maintenance_type;
  
  // تسجيل تفاصيل اللوحة 489 للتحقق
  const billboardId = billboard.ID || billboard.id;
  if (billboardId === 489 || billboardId === '489') {
    console.log('🔍 isBillboardAvailable called for 489:', {
      contractNumber,
      endDate,
      status,
      maintenanceStatus,
      maintenanceType,
      isContractExpired: endDate ? isContractExpired(endDate) : 'no endDate',
      currentDate: new Date().toISOString()
    });
  }
  
  // غير متاحة إذا كانت صيانة/إزالة/خارج الخدمة/مؤجرة صراحةً
  if (
    status === 'صيانة' || status === 'maintenance' ||
    status === 'إزالة' || status === 'ازالة' || status === 'removed' ||
    status === 'مؤجر' || status === 'rented' || status === 'Rented' ||
    ...
    if (billboardId === 489 || billboardId === '489') {
      console.log('❌ Billboard 489 unavailable due to status/maintenance');
    }
    return false;
  }
  ...
  if (!contractNumber) {
    if (billboardId === 489 || billboardId === '489') {
      console.log('✅ Billboard 489 available - no contract');
    }
    return true;
  }
  ...
  if (!endDate) {
    if (billboardId === 489 || billboardId === '489') {
      console.log('❌ Billboard 489 unavailable - contract exists but no end date');
    }
    return false;
  }
  
  // متاحة فقط إذا انتهى العقد
  const expired = isContractExpired(endDate);
  if (billboardId === 489 || billboardId === '489') {
    console.log(expired ? '✅ Billboard 489 available - contract expired' : '❌ Billboard 489 unavailable - contract still active');
  }
  return expired;
};