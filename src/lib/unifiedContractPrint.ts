...
export async function generateUnifiedPrintHTML(options: UnifiedPrintOptions): Promise<string> {
  ...
  const firstPageHTML = `
    <div class="contract-preview-container" style="
      ...
      background: white;
    ">
      <img src="${effectiveBgUrl}" alt="قالب العقد" style="
        position: absolute;
        inset: 0;
        width: 100%;
        height: 100%;
  ...
  const tablePages = billboardPages
    ...
    .map((pageBillboards, pageIndex) => 
      buildTablePageHTML(pageBillboards, settings, effectiveTableBgUrl, pageIndex, qrDataUrls)
    );
...
// ===== OPEN PRINT WINDOW =====
export async function openUnifiedPrintWindow(htmlContent: string): Promise<void> {
  const { showPrintPreview } = await import('@/components/print/PrintPreviewDialog');
  showPrintPreview(htmlContent, 'طباعة العقد');
}
