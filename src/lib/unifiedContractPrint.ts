...
function buildTableTermInlineSvg(tableTerm: NonNullable<PageSectionSettings['tableTerm']>): string {
  const title = tableTerm.termTitle || 'البند الثامن:';
  const content = tableTerm.termContent || 'المواقع المتفق عليها بين الطرفين';
  ...
  const titlePadX = 4;
  const gapPx = 8;
  const height = Math.max(fontSize * 1.55, 28);
  const goldLineHeight = Math.max(height * ((tableTerm.goldLine?.heightPercent || 30) / 100), 2);
  // Calculate top offset so gold line is vertically centered without using transform
  const goldLineTop = Math.round((height - goldLineHeight) / 2);

  // html2canvas-safe: gold line as background-image on the title container itself
  // No child div, no absolute positioning — just background with centered position
  const goldBg = tableTerm.goldLine?.visible !== false
    ? `background-image: url('${solidFillDataUri(tableTerm.goldLine?.color || '#D4AF37')}'); background-repeat: no-repeat; background-position: center center; background-size: 100% ${goldLineHeight}px;`
    : '';

  return `
    <div style="display: inline-flex; flex-direction: row; align-items: center; gap: ${gapPx}px; height: ${height}px; direction: rtl; font-family: 'Doran', sans-serif; line-height: 1;">
      <div style="display: inline-flex; align-items: center; justify-content: center; padding: 0 ${titlePadX}px; height: ${height}px; border-radius: 2px; ${goldBg}">
        <span style="font-size: ${fontSize}px; font-weight: ${titleWeight}; color: ${tableTerm.color || '#1a1a2e'}; white-space: nowrap; direction: rtl;">${title}</span>
      </div>
      <span style="font-size: ${fontSize}px; font-weight: ${contentWeight}; color: ${tableTerm.color || '#1a1a2e'}; white-space: nowrap; direction: rtl;">${content}</span>
    </div>
  `;
}
...
function buildOriginalPriceInlineSvg(originalPrice: string, discountSettings: DiscountDisplaySettings): string {
  ...
  const height = Math.max(fontSize * 1.4, 22);
  const strikeWidth = Math.max(Number(discountSettings.strikethroughWidth || 2), 1);
  // Calculate top offset so strikethrough is vertically centered without using transform
  const strikeTop = Math.round((height - strikeWidth) / 2);

  // html2canvas-safe: strikethrough as background-image on container — no absolute/transform
  return `
    <div style="display: inline-flex; align-items: center; justify-content: center; height: ${height}px; padding: 0 4px; direction: rtl; font-family: 'Doran', sans-serif; background-image: url('${solidFillDataUri(discountSettings.strikethroughColor || '#cc0000')}'); background-repeat: no-repeat; background-position: center center; background-size: 100% ${strikeWidth}px;">
      <span style="font-size: ${fontSize}px; color: ${discountSettings.originalPriceColor || '#888888'}; white-space: nowrap; text-align: center;">${text}</span>
    </div>
  `;
}
...
function buildTablePageHTML(
  ...
  if (pageIndex === 0 && tableTerm?.visible !== false) {
    ...
    const termWrapHeight = Math.max(termFontSize * 1.4, 24);
    const tableTermSvg = buildTableTermInlineSvg(tableTerm as NonNullable<PageSectionSettings['tableTerm']>);
