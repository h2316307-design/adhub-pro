...
export async function generateAccountStatementHTML(data: AccountStatementData): Promise<string> {
  ...
  const periodStart = data.startDate ? new Date(data.startDate).toLocaleDateString('ar-LY') : 'بداية السجل';
  const periodEnd = data.endDate ? new Date(data.endDate).toLocaleDateString('ar-LY') : 'حتى الآن';
  ...
  return wrapInDocument(t, {
    ...
    headerMetaHtml: `
      ...
      تاريخ الإصدار: <span class="num">${new Date().toLocaleDateString('ar-LY')}</span>
    `,
    customerHtml,
    bodyContent,
    extraCSS,
