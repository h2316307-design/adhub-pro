...
export function renderBillboardsTablePage(options: RenderTableOptions): string {
  ...
  if (pageIndex === 0 && showTableTerm && tableTerm?.visible) {
    ...
    const termWrapHeight = Math.max(termFontSize * 1.55, 28);
    const termLiftPx = Math.max(Math.round(termFontSize * 0.28), 4);
    
    tableTermHtml = `
      <div style="
        position: absolute;
        top: ${(tblSettings.topPosition - (tableTerm.marginBottom || 8) - 12)}mm;
        left: 50%;
      ...
      ">
        ...
        ">
          ...
          ">
            ${goldLineHtml}
            <span style="visibility: hidden; display: inline-block; line-height: ${termWrapHeight}px;">${tableTerm.termTitle || 'البند الثامن:'}</span>
            <span style="
              position: absolute;
              inset: 0;
              display: flex;
              ...
              transform: translateY(-${termLiftPx}px);
              z-index: 1;
              line-height: 1;
              white-space: nowrap;
            ">${tableTerm.termTitle || 'البند الثامن:'}</span>
          </span>
          <span style="
            font-weight: ${tableTerm.contentFontWeight || 'normal'};
            display: inline-flex;
            align-items: center;
            height: ${termWrapHeight}px;
            line-height: 1;
          ">${tableTerm.termContent || 'المواقع المتفق عليها بين الطرفين'}</span>
        </h2>
      </div>
    `;
  }
    ...
    tableTermHtml = `
      ...
      ">
        ...
        ">
          ...
          <span style="font-weight: ${(tableTerm.contentFontWeight || 'normal')}; display: inline-flex; align-items: center; height: ${termWrapHeight}px; line-height: 1;">
