...
import { FileText, Search, Eye, Edit, Trash2, Download, Calendar, User, Building, Send } from 'lucide-react';
...
export const ContractsTable = () => {
  ...
  const handleSendAlerts = async () => {
    ...
    for (const { contract, daysLeft } of contractsToAlert) {
      ...
      const status = daysLeft <= 0 ? 'منتهي' : `ينتهي خلال ${daysLeft} يوم`;
      const message = `مرحباً ${customerName},\n\nنود تذكيرك بأن العقد رقم ${contractNumber} ${status}.\n\nنأمل التواصل معنا لتجديد العقد.\n\nشكراً لك.`;
