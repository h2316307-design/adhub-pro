  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl max-h-[92vh] overflow-hidden flex flex-col p-0">
        {/* Header with Steps */}
        <div className="bg-gradient-to-r from-orange-600 to-amber-600 dark:from-orange-800 dark:to-amber-800 text-white p-5">
              ...
              return (
                <div key={step.number} className="flex items-center">
                  <div className="flex flex-col items-center">
                    ...
                    <span className={`text-xs mt-1.5 ${isActive ? 'font-bold' : 'opacity-80'}`}>
                      {step.title}
                    </span>
                  </div>
                  {idx < steps.length - 1 && (
                    <div className={`w-20 h-1 mx-3 mt-[-20px] rounded-full ${isCompleted ? 'bg-green-500' : 'bg-white/30'}`} />
                  )}
                </div>
              );
            })}
