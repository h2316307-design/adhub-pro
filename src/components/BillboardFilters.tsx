...
const QuickStatus: React.FC<QuickStatusProps> = ({ label, icon, isActive, onClick, variant }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "flex items-center gap-2 px-3.5 py-2 rounded-xl border text-xs sm:text-sm font-bold transition-all duration-300",
        "hover:scale-[1.03] active:scale-[0.97] shadow-sm select-none",
        ...
        <CardContent className="relative p-4">
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              ...
              <div className="relative p-3.5 rounded-2xl bg-gradient-to-br from-primary to-indigo-600 shadow-xl shadow-primary/30">
            ...
            <div className="flex-1 relative">
              <SearchInputWithHistory
                ...
                className={cn(
                  "h-14 pr-5 pl-14 text-base bg-background/50 backdrop-blur-md",
                  "border border-border/50 focus:border-primary/50 focus:ring-4 focus:ring-primary/20",
                  "rounded-2xl shadow-inner text-right font-medium",
                  "placeholder:text-muted-foreground/30 transition-all duration-300",
                  "focus:shadow-lg focus:shadow-primary/10 focus:bg-background"
                )}
                dir="rtl"
              />
              {searchQuery && (
      ...
      <div className="flex items-center gap-3 flex-wrap p-3 rounded-[1.5rem] bg-card/60 backdrop-blur-md border border-border/60 shadow-sm">
        ...
        {(searchQuery || activeFiltersCount > 0) && (
          <Button
            ...
            className="gap-1.5 h-9 px-3 rounded-xl text-destructive hover:bg-destructive/10 hover:text-destructive mr-auto text-xs sm:text-sm font-bold transition-all duration-300 active:scale-95"
          >
