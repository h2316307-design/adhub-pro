...
export function CreatePrintTaskFromInstallation({
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const [pricingResult, billboardsResult, designsResult, printersResult, sizesResult] = await Promise.all([
  ...
  const handleCreatePrintTask = async () => {
    try {
      ...
      // إنشاء بنود مهمة الطباعة - بند منفصل لكل لوحة
      ...
      if (cutoutGroups.length > 0 && totals.printerCutoutTotal > 0) {
        ...
        // إنشاء بنود مهمة القص لكل لوحة بها مجسم
        const cutoutTaskItems = cutoutGroups.flatMap(group => 
          (group.cutoutBillboards || []).map(billboardId => ({
            ...
            total_cost: group.printerCutoutCostPerUnit,
