...
export function PrintAllContractBillboardsDialog({
            ...
            <div class="absolute-field" style="top: ${s.installed_images_top}; left: ${s.installed_images_left}; transform: translateX(-50%); width: ${s.installed_images_width}; display: flex; gap: ${s.installed_images_gap};">
              <div style="flex: 1; text-align: center;">
                ...
                <div style="height: ${s.installed_image_height}; overflow: hidden; border: 2px solid #000; border-radius: 8px;">
                  <img src="${effectiveInstalledA}" alt="التركيب - الوجه الأمامي" style="width: 100%; height: 100%; object-fit: contain;" onerror="this.onerror=null;this.src='/placeholder.svg'" />
                </div>
              </div>
              <div style="flex: 1; text-align: center;">
                <div style="font-size: 12px; font-weight: 600; color: #000; margin-bottom: 3mm;">التركيب - الوجه الخلفي</div>
                <div style="height: ${s.installed_image_height}; overflow: hidden; border: 2px solid #000; border-radius: 8px;">
                  <img src="${effectiveInstalledB}" alt="التركيب - الوجه الخلفي" style="width: 100%; height: 100%; object-fit: contain;" onerror="this.onerror=null;this.src='/placeholder.svg'" />
                </div>
              </div>
            </div>
          ` : effectiveInstalledA ? `
            ...
            <div class="absolute-field designs-section" style="top: ${s.designs_top}; left: ${s.designs_left}; width: ${s.designs_width}; display: flex; gap: ${s.designs_gap}; ${effectiveDesignA && !effectiveDesignB ? 'justify-content: center;' : ''}">
              ${effectiveDesignA ? `
                <div class="design-item" ${!effectiveDesignB ? 'style="max-width: 60%;"' : ''}>
                  ...
                  <img src="${effectiveDesignA}" alt="التصميم" class="design-image" style="max-height: ${s.design_image_height};" onerror="this.onerror=null;this.src='/placeholder.svg'" />
                </div>
              ` : ''}
              ${effectiveDesignB ? `
                <div class="design-item" ${!effectiveDesignA ? 'style="max-width: 60%;"' : ''}>
                  ...
                  <img src="${effectiveDesignB}" alt="التصميم" class="design-image" style="max-height: ${s.design_image_height};" onerror="this.onerror=null;this.src='/placeholder.svg'" />
                </div>
              ` : ''}
            </div>
          ` : ''}
        ...
        const cells = finalColumns.map((col, colIndex) => {
          ...
          switch (col.id) {
            ...
            case 'design_images':
              return `<td class="image-cell">
                <div class="img-group">
                  ${designFaceA ? `<img src="${designFaceA}" alt="أمامي" class="design-img" onerror="this.onerror=null;this.src='/placeholder.svg'" />` : ''}
                  ${designFaceB ? `<img src="${designFaceB}" alt="خلفي" class="design-img" onerror="this.onerror=null;this.src='/placeholder.svg'" />` : ''}
            ...
            case 'installed_images':
              return `<td class="image-cell">
                <div class="img-group">
                  ${installedImageFaceA ? `<img src="${installedImageFaceA}" alt="أمامي" class="installed-img" onerror="this.onerror=null;this.src='/placeholder.svg'" />` : ''}
                  ${installedImageFaceB ? `<img src="${installedImageFaceB}" alt="خلفي" class="installed-img" onerror="this.onerror=null;this.src='/placeholder.svg'" />` : ''}
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      // Use hidden iframe to render pages, then capture each page individually
      ...
      // Capture each .page individually and build PDF with jsPDF
      ...
      for (let i = 0; i < pages.length; i++) {
        ...
        // Force explicit size for html2canvas
      ...
      pdf.save(`عقد_${contractNumber}_جميع_اللوحات.pdf`);
      iframe.remove();
          ...
          <div className="flex gap-2 pt-4 border-t flex-wrap">
            ...
            {selectedTeamIds.size === 1 && (() => {
              ...
              const handleSendWhatsApp = () => {
                ...
                let msg = `*مهام التركيب - فريق ${teamName}*\n`;
                msg += `عقد #${contractNumber} - ${adType || customerName}\n\n`;
                ...
                contractItems.forEach(item => {
                  ...
                  itemsByCity[city].push({ item, bb });
                });
                
                Object.entries(itemsByCity).sort().forEach(([city, items]) => {
                  msg += `📍 *${city}*\n`;
                  items.forEach(({ bb }, i) => {
                ...
                msg += `━━━━━━━━━━━━━━━━━\n`;
                msg += `*الإجمالي: ${contractItems.length} لوحة*`;
