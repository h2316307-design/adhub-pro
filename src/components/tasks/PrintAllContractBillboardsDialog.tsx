...
export function PrintAllContractBillboardsDialog({
    ...
    if (open) {
      ...
      const fetchContractData = async () => {
        try {
          ...
          console.log('📋 Fetching contract data for billboard IDs:', billboardIds);
          if (billboardIds.length === 0) return;
          
          const { data: billboardsData } = await supabase
            .from('billboards')
            .select('ID, Contract_Number, Ad_Type')
            .in('ID', billboardIds);
          
          console.log('📋 Billboards data:', billboardsData);
          ...
          if (uniqueContractNumbers.length > 0) {
            ...
            console.log('📋 Ad types map:', adTypesMap);
            setContractAdTypes(adTypesMap);
            
            const mainContractAdType = adTypesMap[contractNumber] || '';
            console.log('📋 Main contract ad type:', mainContractAdType);
  ...
  const generatePrintHTML = async () => {
    ...
    for (const item of sortedItems) {
      ...
      // جلب رقم العقد ونوع الإعلان الخاص بكل لوحة - مع fallback متعدد
      const itemContractNumber = billboard.Contract_Number || contractNumber;
      
      // محاولة الحصول على نوع الإعلان من مصادر متعددة
      let itemAdType = contractAdTypes[itemContractNumber] || '';
      if (!itemAdType) itemAdType = billboard.Ad_Type || billboard.ad_type || '';
      if (!itemAdType) itemAdType = task?.ad_type || '';
      if (!itemAdType) itemAdType = adType || '';
      
      console.log(`📋 Billboard ${item.billboard_id}: Contract ${itemContractNumber}, Ad Type: "${itemAdType}"`);
