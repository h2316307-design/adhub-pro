...
import { MapPin, Navigation, Image as ImageIcon, CheckCircle2, CalendarIcon, PaintBucket, Printer, RotateCcw, Palette, Box } from "lucide-react";
        ...
        {taskDesigns.length > 0 && (
          <div className="mt-3 pt-2 border-t border-border">
            <div className="space-y-2">
              ...
              {/* خيار المجسم */}
              <div className="flex items-center gap-2 mt-2 p-2 bg-accent/10 rounded-lg border border-accent/30 hover:bg-accent/20 transition-colors"
                onClick={(e) => e.stopPropagation()}>
                <Checkbox
                  id={`cutout-${item.id}`}
                  checked={hasCutout}
                  onCheckedChange={handleCutoutChange}
                  className="h-4 w-4 border-2"
                />
                <Label 
                  htmlFor={`cutout-${item.id}`}
                  className="text-xs font-semibold flex items-center gap-1 cursor-pointer flex-1"
                >
                  <Box className="h-3.5 w-3.5 text-accent" />
                  اللوحة بها مجسم (قص خاص)
                </Label>
                {hasCutout && (
                  <Badge variant="secondary" className="text-[9px] px-2 py-0.5 bg-accent/20 text-accent border-accent/30">
                    مجسم
                  </Badge>
                )}
              </div>
