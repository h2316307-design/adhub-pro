          ...
          {allAlertContracts.length === 0 ? (
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="flex flex-col h-full">
              ...
              <TabsContent value="manual" className="px-6 py-4 mt-0 flex-1 overflow-auto">
                ...
                {contractsWithPhone.length === 0 ? (
                  <div className="text-center py-8 text-muted-foreground">
                    لا توجد عقود بأرقام هواتف
                  </div>
                ) : (
                  <ScrollArea className="h-[420px]">
                    <div className="space-y-3 pl-1">
                      {/* قاربت على الانتهاء */}
                      {expiringContracts.filter(c => getContractPhone(c)).length > 0 && (
                        <>
                      ...
                      {/* منتهية */}
                      {expiredContracts.filter(c => getContractPhone(c)).length > 0 && (
                        <>
                          <h4 className="font-semibold text-sm flex items-center gap-2 text-destructive mt-4">
              ...
              <TabsContent value="auto" className="px-6 py-4 mt-0 flex-1 overflow-auto">
                <div className="space-y-4">
                  ...
                  <Button variant="outline" size="sm" onClick={toggleAll} className="w-full">
                    {selectedContracts.size === allAlertContracts.length ? 'إلغاء تحديد الكل' : 'تحديد كل العقود'}
                  </Button>
