    ...
    try {
      ...
      for (const item of clipboardItems) {
        ...
        if (imageType) {
          ...
          const filePath = `designs/${contractId || 'temp'}/${designId}-face-${face}-${Date.now()}.png`;
          const { error } = await supabase.storage.from('billboard-images').upload(filePath, file);
          if (error) { toast.error('فشل رفع الصورة: ' + error.message); return; }
          const { data: urlData } = supabase.storage.from('billboard-images').getPublicUrl(filePath);
          updateDesign(designId, face === 'A' ? { designFaceA: urlData.publicUrl } : { designFaceB: urlData.publicUrl });
