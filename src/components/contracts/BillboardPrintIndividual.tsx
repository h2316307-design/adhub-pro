...
export const BillboardPrintIndividual: React.FC<BillboardPrintIndividualProps> = ({
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      if (!billboards || billboards.length === 0) {
        toast.warning('لا توجد لوحات للتحميل');
        return;
      }
      ...
      for (let i = 0; i < sortedBillboards.length; i++) {
        ...
        const pageHtml = `
          <div style="position: relative; width: 793.7px; height: 1122.5px; margin: 0; padding: 0; overflow: hidden; background: white;">
            ...
            <div <div class="absolute-field ad-type" style="position: absolute;top: 40mm; right: 12mm;font-size: 14px;font-weight: 700;"> 
              عقد رقم: ${contractNumber}
            </div>

            ${adType ? `
              <div class="absolute-field ad-type" style="position: absolute; top: 40mm; right: 35mm; font-size: 14px; font-weight: 700;">
                نوع الإعلان: ${adType}
              </div>
            ` : ''}
