      ...
      <CardContent className="space-y-4">
        <div className="text-sm text-muted-foreground">
          ...
          <span className="text-primary font-medium ml-1">
            {(estimatedTotal || 0).toLocaleString('ar-LY')} د.ل
          </span>
        </div>
        ...
        <div className="space-y-2 border-t border-border pt-3">
          <div className="flex justify-between text-sm">
            ...
            <span className="text-primary font-medium">{(baseTotal || 0).toLocaleString('ar-LY')} د.ل</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">الخصم:</span>
            <span className="text-destructive font-medium">{(discountAmount || 0).toLocaleString('ar-LY')} د.ل</span>
          </div>
          <div className="flex justify-between text-base font-semibold">
            <span className="text-card-foreground">الإجمالي بعد الخصم:</span>
            <span className="text-primary">{(finalTotal || 0).toLocaleString('ar-LY')} د.ل</span>
          </div>
          ...
          {installationCost > 0 && (
            <>
              <div className="flex justify-between text-sm items-center">
                ...
                <span className="text-accent font-medium">{(installationCost || 0).toLocaleString('ar-LY')} د.ل</span>
              </div>
              <div className="flex justify-between text-base font-semibold">
          ...
          {operatingFee > 0 && (
            <div className="flex justify-between text-sm items-center">
              ...
              <span className="text-blue-400 font-medium">{(operatingFee || 0).toLocaleString('ar-LY')} د.ل</span>
            </div>
          )}
          ...
          <div className="flex justify-between text-sm">
            ...
            <span className="text-destructive font-medium">
              {((finalTotal || 0) - (currentContract?.['Total Paid'] || 0)).toLocaleString('ar-LY')} د.ل
            </span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">السابق:</span>
            <span className="text-primary font-medium">{(originalTotal || 0).toLocaleString('ar-LY')} د.ل</span>
            <span className="text-blue-400 font-medium">
              • الفرق: {((finalTotal || 0) - (originalTotal || 0)).toLocaleString('ar-LY')} د.ل
            </span>
          </div>
        </div>
