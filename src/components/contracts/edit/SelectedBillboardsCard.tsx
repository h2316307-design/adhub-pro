          ...
          {selected.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {filteredSelectedBillboards.map((b) => {
                ...
                // ✅ حساب التكاليف المضمنة في السعر (تُخصم من الإيجار الأساسي)
                ...
                // ✅ صافي الإيجار قبل الخصم = الإيجار الأساسي - التكاليف المضمنة
                // مثال: 5200 - 300 = 4900
                ...
                // ✅ حساب الخصم من صافي الإيجار (وليس من الإجمالي الكامل)
                // حساب نسبة هذه اللوحة من إجمالي صافي الإيجار لجميع اللوحات
                const totalNetRentalBeforeDiscount = selectedBillboards.reduce((sum, bb) => {
                  const bbPrice = calculateBillboardPrice(bb);
                  const bbInstall = installationDetails.find(d => d.billboardId === String((bb as any).ID))?.installationPrice || 0;
                  const bbPrint = printCostDetails.find(d => d.billboardId === String((bb as any).ID))?.printCost || 0;
                ...
                // ✅ صافي الإيجار بعد الخصم = صافي الإيجار قبل الخصم - الخصم
                // مثال: 4900 - 1040 = 3860
                const netRentalAfterDiscount = Math.max(0, netRentalBeforeDiscount - discountPerBillboard);
                
                // ✅ التكاليف الإضافية (غير المضمنة) تُضاف بعد الخصم
                ...
                // ✅ الإجمالي الكلي للوحة = صافي الإيجار بعد الخصم + التكاليف المضمنة + التكاليف الإضافية
                  ...
                  >
                    ...
                    <div className="p-4 space-y-4">
                      ...
                      <div className="bg-card border border-border rounded-xl p-4 space-y-3">
                        ...
                        {/* ✅ صافي الإيجار قبل الخصم - يظهر عندما تكون هناك تكاليف مضمنة */}
                        {hasIncludedCosts && (
                          <div className="flex justify-between items-center bg-emerald-500/10 rounded-lg px-3 py-2 -mx-1 border border-emerald-500/20">
                            <span className="text-sm font-bold text-emerald-600">صافي الإيجار (قبل الخصم)</span>
                            <span className="text-lg font-bold text-emerald-600 font-manrope">{netRentalBeforeDiscount.toLocaleString('ar-LY')} {currencySymbol}</span>
                          </div>
                        )}
                        ...
                        {((printCostEnabled && !includePrintInPrice && printCostForBillboard > 0) || 
                          ...
                          <div className="flex justify-between items-center bg-primary/10 rounded-lg px-3 py-2 -mx-1 border border-primary/20">
                            ...
                            <span className="text-lg font-bold text-primary font-manrope">{(netRentalBeforeDiscount + extraInstallCost + extraPrintCost).toLocaleString('ar-LY')} {currencySymbol}</span>
                          </div>
                        )}
                        ...
                        {discountPerBillboard > 0 && (
                          <>
                            ...
                            <div className="flex justify-between items-center bg-green-500/10 rounded-lg px-3 py-2 -mx-1 border border-green-500/20">
                              <span className="text-sm font-bold text-green-600">صافي الإيجار (بعد الخصم)</span>
                              <span className="text-lg font-bold text-green-600 font-manrope">{netRentalAfterDiscount.toLocaleString('ar-LY')} {currencySymbol}</span>
                            </div>
                            {/* Total after discount including costs */}
                            <div className="flex justify-between items-center bg-primary/10 rounded-lg px-3 py-2 -mx-1 border border-primary/30">
                              ...
                              <div className="text-left">
                                <span className="text-lg font-bold text-primary font-manrope">
                                  {totalForBoard.toLocaleString('ar-LY')} {currencySymbol}
                                </span>
