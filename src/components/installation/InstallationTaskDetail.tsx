...
import {
  ...
  Calendar as CalendarIcon, Layers, Wrench, Search, X, Archive,
  LayoutGrid, Rows3
} from 'lucide-react';
...
export const InstallationTaskDetail: React.FC<Props> = ({
  ...
  const [compactCards, setCompactCards] = useState(() => localStorage.getItem('installation-task-card-density') === 'compact');
  const [inlineDate] = useState<Date | undefined>(undefined);

  const cardGridClass = compactCards
  ...
  const toggleCompactCards = () => {
    setCompactCards(prev => {
      const next = !prev;
      localStorage.setItem('installation-task-card-density', next ? 'compact' : 'normal');
      return next;
    });
  };
      ...
      <div className="flex flex-col lg:flex-row">
        ...
        <div className="w-full lg:w-80 xl:w-96 shrink-0 border-b lg:border-b-0 lg:border-l border-border bg-card flex flex-col">
          ...
          <div className="p-4 space-y-4 flex-1">
            <h3 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">الإجراءات</h3>
            <div className="space-y-2">
              <p className="text-[10px] font-semibold text-muted-foreground">التصاميم واللوحات</p>
            <Button variant="outline" size="sm" onClick={onManageDesigns} className="w-full justify-start gap-2 h-9 text-xs border-border">
              <Edit className="h-3.5 w-3.5" />
              إدارة التصاميم
            </Button>
            <Button variant="outline" size="sm" onClick={onDistributeDesigns} className="w-full justify-start gap-2 h-9 text-xs border-border">
              <Layers className="h-3.5 w-3.5 text-amber-400" />
              توزيع التصاميم
            </Button>
            <Button variant="outline" size="sm" onClick={onAddBillboards} className="w-full justify-start gap-2 h-9 text-xs border-border">
              <Plus className="h-3.5 w-3.5" />
              إضافة لوحات
            </Button>
            <Button variant="outline" size="sm" onClick={onCompleteBillboards} className="w-full justify-start gap-2 h-9 text-xs border-border">
            ...
            <div className="space-y-2">
              <p className="text-[10px] font-semibold text-muted-foreground">الطباعة والتصدير</p>
            <Button variant="outline" size="sm" onClick={onPrintAll} className="w-full justify-start gap-2 h-9 text-xs border-border">
              <Printer className="h-3.5 w-3.5" />
              طباعة الكل
            </Button>
            <Button variant="outline" size="sm" onClick={onExportZip} className="w-full justify-start gap-2 h-9 text-xs border-border text-emerald-500 hover:text-emerald-500">
              <Archive className="h-3.5 w-3.5" />
              تنزيل الصور ZIP
            </Button>
            <Button variant="outline" size="sm" onClick={onCreatePrintTask} className="w-full justify-start gap-2 h-9 text-xs border-border" disabled={!!task.print_task_id}>
            ...
            <div className="space-y-2">
              <p className="text-[10px] font-semibold text-muted-foreground">إدارة المهمة</p>
            <Button variant="outline" size="sm" onClick={onTransferBillboards} className="w-full justify-start gap-2 h-9 text-xs border-border">
              <ArrowRight className="h-3.5 w-3.5" />
              نقل لوحات
            </Button>
            {onCreateCompositeTask && (
              <Button variant="outline" size="sm" onClick={onCreateCompositeTask} className="w-full justify-start gap-2 h-9 text-xs border-border">
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-4 p-4 lg:p-6">
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            <Button variant={compactCards ? 'default' : 'outline'} size="sm" onClick={toggleCompactCards} className="h-10 gap-2 shrink-0">
              {compactCards ? <Rows3 className="h-4 w-4" /> : <LayoutGrid className="h-4 w-4" />}
              {compactCards ? 'عرض عادي' : 'تصغير الكروت'}
            </Button>
          </div>
