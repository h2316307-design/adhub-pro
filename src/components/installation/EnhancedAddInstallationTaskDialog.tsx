...
export function EnhancedAddInstallationTaskDialog({
  ...
  const { data: customerContracts = [], isLoading: loadingContracts } = useQuery({
    ...
    queryFn: async () => {
      ...
      // First get contracts by customer_id
      let query = supabase
        .from('Contract')
        .select('Contract_Number, "Customer Name", customer_id, "Ad Type", "End Date", "Contract Date", billboard_ids')
        .eq('customer_id', selectedCustomerId);
      
      // لا تعرض العقود المنتهية (في كل أنواع المهام)
      query = query.gte('"End Date"', today);
      
      const { data, error } = await query.order('Contract_Number', { ascending: false }).limit(100);
  ...
  const { data: contractBillboards = [], isLoading: loadingBillboards } = useQuery({
    ...
    queryFn: async () => {
      ...
      for (const contractId of selectedContractIds) {
        const contract = customerContracts.find(c => c.Contract_Number === contractId);
        if (contract?.billboard_ids) {
          const ids = contract.billboard_ids.split(',').map(id => parseInt(id.trim())).filter(Boolean);
          ids.forEach(id => allBillboardIds.push({ id, contractId }));
        }
      }
              ...
              <div className="space-y-3">
                ...
                {loadingContracts ? (
                  ...
                  <div className="text-center py-4 text-muted-foreground">
                    {contractSearchTerm ? 'لا توجد نتائج للبحث' : 'لا توجد عقود لهذا الزبون'}
                  </div>
                ) : (
