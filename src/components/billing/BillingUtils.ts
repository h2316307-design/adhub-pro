...
export const calculateTotalRemainingDebt = (
  ...
  // إجمالي العقود
  const totalContracts = contracts.reduce((sum, c) => sum + (Number((c as any)['Total']) || 0), 0);
  ...
  // إجمالي الديون
  const totalDebits = totalContracts + totalSalesInvoices + totalPrintedInvoices + totalOtherDebts;
  ...
  // إجمالي المشتريات من الزبون (غير المستخدمة كدفعات)
