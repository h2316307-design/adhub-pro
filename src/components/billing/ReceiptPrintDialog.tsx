...
export default function ReceiptPrintDialog({ open, onOpenChange, payment, customerName }: ReceiptPrintDialogProps) {
  ...
  const calculateRemainingBalance = async () => {
    try {
      ...
      // جلب جميع العقود للعميل
      const { data: contracts, error: contractsError } = await supabase
        .from('Contract')
        .select('Contract_Number, Total, "Total Rent"')
        .eq('customer_id', customerId);

      if (contractsError || !contracts) return null;

      // حساب إجمالي العقود
      const totalContracts = contracts.reduce((sum, c) => 
        sum + (Number((c as any).Total ?? c['Total Rent'] ?? 0) || 0), 0);

      // جلب جميع الدفعات حتى هذه الدفعة (بناءً على التاريخ)
      const paymentDate = payment.paid_at || new Date().toISOString();
      
      const { data: payments, error: paymentsError } = await supabase
        .from('customer_payments')
        .select('amount, entry_type, paid_at')
        .eq('customer_id', customerId)
        .lte('paid_at', paymentDate)
        .order('paid_at', { ascending: true });

      if (paymentsError || !payments) return null;

      // حساب إجمالي المدفوعات والديون حتى هذه الدفعة
      let totalPaid = 0;
      let totalDebts = 0;
      ...
      for (const p of payments) {
        ...
        if (p.entry_type === 'receipt' || p.entry_type === 'account_payment') {
          totalPaid += amount;
        } else if (p.entry_type === 'debt' || p.entry_type === 'invoice') {
          totalDebts += amount;
        }
      }

      // الرصيد المتبقي = (إجمالي العقود + الديون) - المدفوع
      const remainingBalance = (totalContracts + totalDebts) - totalPaid;
            ...
            <div class="payment-details">
              ...
              <div class="payment-info">
                ...
                <div>
                  ...
                  ${payment.contract_number || 'غير محدد'}
                </div>
                ${payment.notes ? `
