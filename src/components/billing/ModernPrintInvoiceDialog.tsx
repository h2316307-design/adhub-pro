...
export default function ModernPrintInvoiceDialog({
            ...
            <div class="invoice-container">
              ...
              ${showNotes && notes ? `
                <div style="margin-top: 20px; padding: 15px; background: #f8f9fa; border: 1px solid #ddd; border-radius: 4px;">
                  <strong style="font-size: 13px;">ملاحظات:</strong> <span style="font-size: 12px;">${notes}</span>
                </div>
              ` : ''}
  ...
  const handleDownloadPDF = async (forPrinter: boolean = false) => {
    ...
    try {
      ...
      // إنشاء HTML باستخدام نفس المحتوى من دالة handlePrint
      const currentDate = new Date(invoiceDate);
      const formattedDate = currentDate.toLocaleDateString('ar-LY');
      const contractsList = selectedContracts.join('-');
      const dateFormatted = currentDate.toISOString().slice(0, 10).replace(/-/g, '_');
      ...
      // توليد HTML (مطابق لدالة handlePrint)
      const htmlContent = generateModernPrintInvoiceHTML({
        customerName,
        invoiceNumber,
        invoiceDate: formattedDate,
        printItems: displayItems,
        moneySubtotal,
        ...
        currency,
        notes,
        forPrinter: isPrinterCopy,
        invoiceType,
        facesTotal,
        showNotes, // ✅ إضافة showNotes
      });
