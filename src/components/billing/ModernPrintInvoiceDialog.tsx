...
export default function ModernPrintInvoiceDialog({
  ...
  const handlePrint = async (isPrinterCopyParam: boolean = false) => {
    ...
    try {
      ...
      const invoiceTypeTextShort = invoiceType === 'print_only' ? 'طباعة' : invoiceType === 'print_install' ? 'طباعة_وتركيب' : 'تركيب';
      const customerNameForFile = customerName.replace(/[^a-zA-Z0-9\u0600-\u06FF]/g, '_');
      const dateFormatted = new Date(invoiceDate).toISOString().slice(0, 10).replace(/-/g, '_');
      const fileName = `فاتورة_${invoiceTypeTextShort}_${customerNameForFile}_${dateFormatted}`;
