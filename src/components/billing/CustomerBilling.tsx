...
export default function CustomerBilling() {
  ...
  // State for friend billboard rentals
  const [totalFriendRentals, setTotalFriendRentals] = useState(0);
    ...
    try {
      ...
      try {
        if (customerId) {
          ...
          console.log('Customer linked_friend_company_id:', customerInfo?.linked_friend_company_id);
          
          if (customerInfo?.linked_friend_company_id) {
            const { data: rentalsData, error: rentalsError } = await supabase
              .from('friend_billboard_rentals')
              .select('friend_rental_cost')
              .eq('friend_company_id', customerInfo.linked_friend_company_id);
            
            console.log('Friend rentals data:', rentalsData, 'Error:', rentalsError);
            
            if (rentalsData) {
              friendRentalsTotal = rentalsData.reduce((sum, r) => sum + (Number(r.friend_rental_cost) || 0), 0);
              console.log('Total friend rentals:', friendRentalsTotal);
            }
          }
        }
      } catch (e) {
      ...
      setTotalFriendRentals(friendRentalsTotal);
      ...
      if (customerId) {
        try {
          ...
          if (!discountsError && discountsData) {
            const discountSum = discountsData.reduce((sum, discount) => {
              return sum + (Number(discount.discount_value) || 0);
            }, 0);
  ...
  // ✅ FIXED: Calculate financial totals including all invoice types
  const totalRent = useMemo(() => contracts.reduce((s, c) => s + (Number((c as any)['Total']) || 0), 0), [contracts]);

  // ✅ حساب إجمالي فواتير الطباعة
  const totalPrintedInvoicesAmount = useMemo(() => {
    return printedInvoices.reduce((sum, inv) => {
      const val = (inv as any).total_amount ?? (inv as any).print_cost ?? 0;
  ...
  const totalDebits = useMemo(() => {
    ...
    // إضافة الديون من الدفعات (استثناء ما هو مرتبط بفواتير المبيعات لأنها محتسبة مباشرة)
    payments.forEach(p => {
      const amount = Number(p.amount) || 0;
      // ✅ FIX: استثناء السجلات المرتبطة بفواتير المبيعات (sales_invoice_id موجود)
      if ((p.entry_type === 'invoice' || p.entry_type === 'debt') && !(p as any).sales_invoice_id && !(p as any).printed_invoice_id) {
        totalDebit += amount;
      }
    });
    
    // ✅ إضافة فواتير المبيعات من جدول sales_invoices
    salesInvoices.forEach(inv => {
      totalDebit += Number(inv.total_amount) || 0;
    });
    
    // ✅ إضافة فواتير الطباعة
    totalDebit += totalPrintedInvoicesAmount;
    
    console.log('totalDebits calculation:', {
      totalRent,
      totalPrintedInvoicesAmount,
      totalFriendRentals,
      beforeSubtract: totalDebit,
      afterSubtract: totalDebit - totalFriendRentals
    });
    
    // ✅ FIX: طرح إيجارات اللوحات الصديقة من الديون (لأنها خصم من المستحقات)
    totalDebit -= totalFriendRentals;
    
    return totalDebit;
  }, [payments, totalRent, salesInvoices, totalPrintedInvoicesAmount, totalFriendRentals]);

  const totalCredits = useMemo(() => {
    return payments.reduce((s, p) => {
      const amount = Number(p.amount) || 0;
      // ✅ FIXED: احتساب جميع أنواع المدفوعات بما فيها فواتير المبيعات والطباعة
      if (p.entry_type === 'receipt' || p.entry_type === 'account_payment' || p.entry_type === 'payment' || 
          p.entry_type === 'sales_invoice' || p.entry_type === 'purchase_invoice') {
        return s + amount;
      }
      return s;
    }, 0);
  ...
  // حساب مجموع الخصومات - يتم تحميلها في loadData
  const [totalDiscounts, setTotalDiscounts] = useState(0);
  
  // حساب المتبقي مع الخصومات
  const balance = totalDebits - totalCredits - totalDiscounts;
  ...
  return (
    <div className="expenses-container">
      ...
      <SummaryCards
        ...
        totalPrintedInvoices={totalPrintedInvoicesAmount}
