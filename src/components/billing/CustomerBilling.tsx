...
export default function CustomerBilling() {
  ...
  const totalDebits = useMemo(() => {
    ...
    payments.forEach(p => {
      ...
      if (p.entry_type === 'invoice' || p.entry_type === 'debt' || p.entry_type === 'sales_invoice' || p.entry_type === 'purchase_invoice') {
        totalDebit += amount;
      }
    });
    return totalDebit;
  ...
  const getContractPayments = (contractNumber: number | string): number => {
    ...
    return payments
      .filter(p => {
        ...
        const isValidType = p.entry_type === 'receipt' || p.entry_type === 'account_payment';
