        ...
        <div className="space-y-3">
          {overduePayments.map((payment, index) => (
            <div
              ...
              className="flex items-center justify-between p-4 border border-destructive/20 rounded-lg bg-destructive/5 hover:bg-destructive/10 transition-colors"
            >
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2">
                  <Badge variant="destructive">{payment.daysOverdue} يوم تأخير</Badge>
                  <span className="font-bold text-foreground">عقد #{payment.contractNumber}</span>
                </div>
                <div className="text-sm text-muted-foreground">
                  <div><strong>العميل:</strong> {payment.customerName}</div>
                  <div><strong>المبلغ المستحق:</strong> {payment.installmentAmount.toLocaleString('ar-LY')} د.ل</div>
                  <div><strong>تاريخ الاستحقاق:</strong> {new Date(payment.dueDate).toLocaleDateString('ar-LY')}</div>
