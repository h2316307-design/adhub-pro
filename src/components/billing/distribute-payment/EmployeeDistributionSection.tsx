...
export function EmployeeDistributionSection({
  ...
  // المبلغ الذي يجلبه زر "كامل": المتبقي من الدفعة الكلية + ما هو مُدخل حالياً،
  // مقيّداً برصيد الموظف عند السحب من الرصيد، وبسقف القسم.
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div className="flex items-center gap-1.5">
                            <Input
                              ...
                              placeholder="المبلغ"
                              className="h-8 text-xs text-left flex-1 disabled:opacity-60"
                              dir="ltr"
                              disabled={Number.isFinite(computeMax(distribution)) && computeMax(distribution) <= 0 && Number(distribution.amount || 0) === 0}
                            />
                            ...
                            <Button
                              ...
                              size="sm"
                              variant="outline"
                              onClick={() => updateEmployeePaymentDistribution(index, 'amount', computeFillAmount(distribution))}
                              className="text-[10px] h-7 border-green-300 text-green-700 hover:bg-green-50"
                              disabled={computeFillAmount(distribution) <= 0}
                              title={`المتبقي: ${computeFillAmount(distribution).toFixed(2)}`}
                            >
                              كامل
                            </Button>
                          </div>
                          {Number.isFinite(computeMax(distribution)) && computeMax(distribution) <= 0 && Number(distribution.amount || 0) === 0 && (
                            <div className="text-[10px] text-red-600 font-semibold">⛔ تم بلوغ سقف الدفعة</div>
                          )}
                          {Number.isFinite(computeMax(distribution)) && (
                            <div className="text-[10px] text-muted-foreground flex justify-between">
                              <span>الحد الأقصى المتاح:</span>
                              <span className="font-bold text-green-700">{computeMax(distribution).toFixed(2)} د.ل</span>
                            </div>
                          )}
