...
export function UnifiedPrintAllDialog({
  ...
  const filteredItems = useMemo(() => {
    if (!showTeamFilter || selectedTeamIds.size === 0) return items;
    return items.filter(item => {
      const teamId = item.team_id || 'unknown';
      return selectedTeamIds.has(teamId);
    });
  }, [items, selectedTeamIds, showTeamFilter]);
  ...
  const generatePrintHTML = async () => {
    ...
    for (const item of sortedItems) {
      ...
      // For removal tasks, show item-specific contract info if available
      ...
      <head>
        ...
        <style>
          ...
          * { margin: 0; padding: 0; box-sizing: border-box; }
          ...
          .billboard-name { font-family: 'Manrope', Arial, sans-serif; font-size: 20px; font-weight: 500; color: #333; }
          .size { font-family: 'Manrope', Arial, sans-serif; font-size: 41px; font-weight: 500; }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        ...
        <div className="space-y-4">
          {/* اختيار الفرق */}
          {showTeamFilter && Object.keys(itemsByTeam).length > 1 && (
                ...
                {Object.entries(itemsByTeam).map(([teamId, teamItems]) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <Badge variant="secondary" className="text-xs">{teamItems.length}</Badge>
                    </button>
                  );
                })}
              </div>
