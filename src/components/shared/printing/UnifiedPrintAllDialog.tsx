    ...
    sortedItems.forEach(item => {
      ...
      enabledColumns.forEach(col => {
        switch (col.id) {
          ...
          case 'installation_date': if (item.installation_date) columnHasData[col.id] = true; break;
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const options = {
        ...
        jsPDF: { unit: 'mm' as const, format: 'a4' as const, orientation: isLandscape ? ('landscape' as const) : ('portrait' as const) },
