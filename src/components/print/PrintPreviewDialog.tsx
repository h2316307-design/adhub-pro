...
export function PrintPreviewDialog() {
  ...
  // Toggle signature section visibility in HTML
  const getProcessedHtml = useCallback((html: string, showSig: boolean) => {
    if (showSig) return html;
    return html.replace('</head>', '<style>.signature-stamp-section { display: none !important; }</style></head>');
  }, []);
