  ...
  useEffect(() => {
    const initMap = () => {
      ...
      filteredBillboards.forEach((b) => {
        ...
        // Check if billboard is rented
        const isRented = label === 'مؤجر';
          ...
          content: `
            <div style="min-width: 280px; font-family: 'Cairo', sans-serif; direction: rtl; padding: 0;">
              ...
              <div style="background: ${color}; padding: 10px; border-radius: 8px; margin-bottom: 10px;">
                ...
                ` : ''}
                ${isRented && adType ? `
                  <div style="margin-top: 4px; font-size: 11px; color: rgba(255,255,255,0.8);">
                    📢 ${adType}
                  </div>
                ` : ''}
              </div>
      ...
      if (hasMarkers && bounds && !bounds.isEmpty()) {
        try {
