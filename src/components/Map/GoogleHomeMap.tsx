...
export default function GoogleHomeMap({ 
  ...
  }, [billboards, searchQuery, externalSearchQuery, externalStatusFilter, externalCityFilter, externalSizeFilter, externalMunicipalityFilter]);
  ...
  // Adds fading effect for visited billboards (within 50m during tracking)
  // Smaller pins on mobile for better UX
  const createPinWithLabel = useCallback((billboard: Billboard, isSelected: boolean = false, isVisited: boolean = false) => {
    ...
    const statusColor = isVisited ? '#22c55e' : status.color;
    const isAvailable = status.label === 'متاحة' || status.label === 'متاح';
    
    // إزالة تأثير الأوباسيتي الذي يسبب مشاكل في التتبع
    const opacity = 1;
    
    // مقياس التصغير للهاتف (0.70 = 70% من الحجم الأصلي) - مثل المستودع المرجعي
    const scale = isMobile ? 0.70 : 1;
    
    // الأبعاد الأساسية (بحجم الديسكتوب) ثم نضربها بالـ scale
    const basePinSize = Math.round((isSelected ? 54 : 44) * scale);
    const pinSize = basePinSize;
    const customerHeight = customerName && isRented ? Math.round(28 * scale) : 0;
    // عرض أكبر للهاتف لاستيعاب النصوص - مثل المستودع المرجعي
    const width = Math.round((isMobile ? 280 : 240) * scale);

    const sizeLabelWidth = Math.round((isMobile ? 200 : 160) * scale);
    const sizeLabelHeight = Math.round(22 * scale);
    const adTypeLabelWidth = Math.round((isMobile ? 220 : 180) * scale);
    const adTypeLabelHeight = Math.round(20 * scale);
    const customerLabelWidth = Math.round((isMobile ? 230 : 190) * scale);
    const showAdType = isRented && !!adType;

    const labelHeight = Math.round(46 * scale);
    const bottomPad = Math.round(46 * scale);
    const totalHeight = labelHeight + pinSize + bottomPad + customerHeight;
    ...
    // قص النص حسب الحجم - توسيع أكثر للهاتف لعرض المزيد من النص - مثل المستودع المرجعي
    const maxLen = isMobile ? 18 : 20;
    const shortSize = size.length > maxLen ? size.substring(0, maxLen - 1) + '..' : size;
    const shortAdType = adType ? (adType.length > maxLen ? adType.slice(0, maxLen - 1) + '..' : adType) : '';
    const shortCustomer = customerName
      ? (customerName.length > (isMobile ? 20 : 22) ? customerName.substring(0, (isMobile ? 19 : 21)) + '..' : customerName)
      : '';
    ...
    // حسابات متناسبة للمسارات
    const pinBottomY = totalHeight - customerHeight - Math.round(8 * scale);
    const pinControlOffset = Math.round(4 * scale);
    const pinControlOffset2 = Math.round(18 * scale);
    const headRPlus = headR + Math.round(4 * scale);
    const headYOffset = Math.round(8 * scale);
    const groundShadowY = totalHeight - customerHeight - Math.round(6 * scale);
    const strokeWidth = Math.max(1, Math.round(1.5 * scale));
    const selectedStrokeWidth = Math.max(2, Math.round(3 * scale));
    
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${totalHeight}" viewBox="0 0 ${width} ${totalHeight}" style="opacity: ${opacity}">
      <defs>
        <linearGradient id="body${uniqueId}" x1="0%" y1="0%" x2="0%" y2="100%">
          <stop offset="0%" stop-color="${adjustColor(sizeColors.bg, 50)}"/>
          <stop offset="50%" stop-color="${sizeColors.bg}"/>
          <stop offset="100%" stop-color="${adjustColor(sizeColors.bg, -40)}"/>
        </linearGradient>
        <filter id="shadow${uniqueId}" x="-50%" y="-30%" width="200%" height="180%">
          <feDropShadow dx="0" dy="${Math.round(3 * scale)}" stdDeviation="${Math.round(3 * scale)}" flood-color="rgba(0,0,0,0.45)"/>
        </filter>
        <filter id="textShadow${uniqueId}" x="-50%" y="-50%" width="200%" height="200%">
          <feDropShadow dx="0" dy="${Math.round(1 * scale)}" stdDeviation="${Math.round(1 * scale)}" flood-color="rgba(0,0,0,0.7)"/>
        </filter>
      ...
      <g filter="url(#textShadow${uniqueId})">
        <rect x="${cx - sizeLabelWidth/2}" y="${Math.round(2 * scale)}" width="${sizeLabelWidth}" height="${sizeLabelHeight}" rx="${sizeLabelHeight/2}" fill="${sizeColors.bg}" stroke="rgba(255,255,255,0.5)" stroke-width="${strokeWidth}"/>
        <text x="${cx}" y="${sizeLabelHeight}" text-anchor="middle" font-family="Tajawal, Arial, sans-serif" font-size="${fontSize}" font-weight="bold" fill="#fff">${shortSize}</text>
      </g>
      
      ${shortAdType && showAdType ? `
      <!-- Ad Type label below size -->
      <g filter="url(#textShadow${uniqueId})">
        <rect x="${cx - adTypeLabelWidth/2}" y="${sizeLabelHeight + Math.round(4 * scale)}" width="${adTypeLabelWidth}" height="${adTypeLabelHeight}" rx="${adTypeLabelHeight/2}" fill="${statusColor}" stroke="rgba(255,255,255,0.4)" stroke-width="${strokeWidth}"/>
        <text x="${cx}" y="${sizeLabelHeight + adTypeLabelHeight}" text-anchor="middle" font-family="Tajawal, Arial, sans-serif" font-size="${smallFontSize}" font-weight="700" fill="#fff">${shortAdType}</text>
      </g>
      ` : ''}
      
      <!-- Ground shadow -->
      <ellipse cx="${cx}" cy="${groundShadowY}" rx="${pinSize * 0.25}" ry="${Math.round(4 * scale)}" fill="rgba(0,0,0,0.25)"/>
      
      ${isAvailable ? `
      <!-- Pulse rings for available -->
      <circle cx="${cx}" cy="${pinTop + headR + headYOffset}" r="${headR + Math.round(5 * scale)}" fill="none" stroke="${statusColor}" stroke-width="${Math.round(2 * scale)}" opacity="0.5">
        <animate attributeName="r" values="${headR + Math.round(2 * scale)};${headR + Math.round(14 * scale)}" dur="1.5s" repeatCount="indefinite"/>
      ...
      <g filter="url(#shadow${uniqueId})">
        <!-- Pin body - teardrop -->
        <path d="M${cx},${pinBottomY} 
                 C${cx - pinControlOffset},${pinBottomY - Math.round(10 * scale)} ${cx - headRPlus},${pinTop + headR + pinControlOffset2} ${cx - headRPlus},${pinTop + headR + headYOffset}
                 A${headRPlus},${headRPlus} 0 1,1 ${cx + headRPlus},${pinTop + headR + headYOffset}
                 C${cx + headRPlus},${pinTop + headR + pinControlOffset2} ${cx + pinControlOffset},${pinBottomY - Math.round(10 * scale)} ${cx},${pinBottomY}Z"
              fill="url(#body${uniqueId})" 
              stroke="${isSelected ? '#fbbf24' : 'rgba(255,255,255,0.5)'}" 
              stroke-width="${isSelected ? selectedStrokeWidth : strokeWidth}"/>
        
        <!-- Shine highlight -->
        <ellipse cx="${cx - headR * 0.35}" cy="${pinTop + headR * 0.6 + headYOffset}" rx="${headR * 0.4}" ry="${headR * 0.3}" 
                 fill="url(#shine${uniqueId})" opacity="0.7"/>
        
        <!-- Inner circle background -->
        <circle cx="${cx}" cy="${pinTop + headR + headYOffset}" r="${headR * 0.7}" fill="#1a1a2e" opacity="0.35"/>
        
        <!-- Status indicator -->
        <circle cx="${cx}" cy="${pinTop + headR + headYOffset}" r="${headR * 0.55}" fill="url(#glow${uniqueId})"/>
        
        <!-- Inner white dot -->
        <circle cx="${cx}" cy="${pinTop + headR + headYOffset}" r="${headR * 0.2}" fill="rgba(255,255,255,0.95)"/>
      </g>
      
      ${shortCustomer && isRented ? `
      <!-- Customer name BELOW pin -->
      <g filter="url(#textShadow${uniqueId})">
        <rect x="${cx - customerLabelWidth/2}" y="${totalHeight - Math.round(26 * scale)}" width="${customerLabelWidth}" height="${Math.round(24 * scale)}" rx="${Math.round(12 * scale)}" fill="rgba(26,26,46,0.95)" stroke="#fbbf24" stroke-width="${strokeWidth * 0.5}"/>
        <text x="${cx}" y="${totalHeight - Math.round(10 * scale)}" text-anchor="middle" font-family="Tajawal, Arial, sans-serif" font-size="${Math.round(13 * scale)}" font-weight="700" fill="#fbbf24">${shortCustomer}</text>
      </g>
      ` : ''}
      
      ${isSelected ? `
      <!-- Selection indicator with rotation -->
      <circle cx="${cx}" cy="${pinTop + headR + headYOffset}" r="${headR + Math.round(10 * scale)}" fill="none" stroke="#fbbf24" stroke-width="${selectedStrokeWidth}" stroke-dasharray="${Math.round(6 * scale)},${Math.round(4 * scale)}" opacity="0.9">
        <animateTransform attributeName="transform" type="rotate" from="0 ${cx} ${pinTop + headR + headYOffset}" to="360 ${cx} ${pinTop + headR + headYOffset}" dur="5s" repeatCount="indefinite"/>
      </circle>
      <!-- Golden checkmark badge -->
      <g>
        <circle cx="${cx + headR + Math.round(2 * scale)}" cy="${pinTop + Math.round(2 * scale)}" r="${Math.round(11 * scale)}" fill="#fbbf24">
          <animate attributeName="r" values="${Math.round(11 * scale)};${Math.round(13 * scale)};${Math.round(11 * scale)}" dur="0.8s" repeatCount="indefinite"/>
        </circle>
        <path d="M${cx + headR - Math.round(2 * scale)} ${pinTop + Math.round(2 * scale)} l${Math.round(4 * scale)} ${Math.round(4 * scale)} l${Math.round(6 * scale)} ${-Math.round(6 * scale)}" stroke="#000" stroke-width="${Math.round(2.5 * scale)}" fill="none" stroke-linecap="round" stroke-linejoin="round"/>
      </g>
      ` : ''}
    </svg>`;
  ...
  // Detailed map style - shows all place names and details
  const detailedMapStyles: google.maps.MapTypeStyle[] = [
    { elementType: 'geometry', stylers: [{ color: '#1a1a1a' }] },
    { elementType: 'labels.text.stroke', stylers: [{ color: '#0a0a0a' }, { weight: 2 }] },
    { elementType: 'labels.text.fill', stylers: [{ color: '#ffffff' }] },
    { featureType: 'administrative.country', elementType: 'labels.text.fill', stylers: [{ color: '#fbbf24' }] },
    { featureType: 'administrative.province', elementType: 'labels.text.fill', stylers: [{ color: '#d4af37' }] },
    { featureType: 'administrative.locality', elementType: 'labels.text.fill', stylers: [{ color: '#fbbf24' }, { weight: 0.8 }] },
    { featureType: 'administrative.neighborhood', elementType: 'labels.text.fill', stylers: [{ color: '#f59e0b' }] },
    ...
    { featureType: 'road.highway', elementType: 'geometry', stylers: [{ color: '#d4af37' }, { lightness: -30 }] },
  ...
  useEffect(() => {
    ...
    const initGoogleMap = () => {
      ...
      const mapTypeId = mapType === 'styled' ? 'roadmap' : mapType;
  ...
  const updateGoogleMarkers = useCallback((skipFitBounds: boolean = false) => {
    ...
    // تعطيل fitBounds أثناء التتبع المباشر لمنع الزوم المتقطع
    if (hasMarkers && !isTracking) {
      try {
        setTimeout(() => {
  ...
  return (
    ...
    >
      ...
      {!isTracking && (
        <div className={`absolute z-[1000] pointer-events-auto ${isMobile ? 'top-2 right-2' : 'top-4 right-4'}`}>
        ...
        <div className={`absolute z-[2000] bg-card/95 backdrop-blur-md border border-border/50 shadow-lg animate-fade-in pointer-events-auto ${
          ...
          <div className="space-y-0.5">
            {[
              ...
              { type: 'detailed' as const, label: 'تفصيلية' }
            ].map((layer) => (
              <button
                key={layer.type}
