...
export default function GoogleHomeMap({ 
        ...
        (pos) => {
          ...
          if (googleMapInstanceRef.current) {
            ...
            if (googleUserMarkerRef.current) {
              googleUserMarkerRef.current.setPosition(loc);
            } else {
          ...
          if (leafletMapInstanceRef.current) {
            leafletMapInstanceRef.current.panTo([loc.lat, loc.lng]);
          }
        },
        (err) => {
  ...
  const filteredBillboards = useMemo(() => {
    ...
    return billboards.filter(b => {
      ...
      if (!parsedCoords) {
        // Debug: log billboards with coordinates that fail parsing
        const rawCoords = (b as any).GPS_Coordinates || (b as any).coordinates || '';
        if (rawCoords && rawCoords !== '0' && rawCoords !== '') {
          console.warn(`[Map] Billboard ${(b as any).ID || (b as any).Billboard_Name} has coords "${rawCoords}" but parseCoords returned null`);
        }
        return false;
      }
