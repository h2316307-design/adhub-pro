...
export function HomeMap({ billboards, onBillboardClick }: HomeMapProps) {
  ...
  useEffect(() => {
    ...
    filteredBillboards.forEach((billboard) => {
      ...
      // ✅ FIX: Check if contract is expired before determining availability
      let isAvailable = statusVal === 'متاح' || (!billboard.Contract_Number && statusVal !== 'صيانة');
      
      // Check if has contract but it's expired
      if (billboard.Contract_Number) {
        ...
        if (rentEndDate) {
          ...
          if (isExpired) {
            isAvailable = true; // Expired contract = Available
          } else {
            isAvailable = false; // Active contract = Not available
          }
        }
      }
      ...
      // Create custom marker with ad type label and enhanced design
      const iconHtml = `
        <div style="
          display: flex;
        ...
        ">
          ...
          <div style="
            ...
            height: 46px;
            background: ${isAvailable 
              ? 'linear-gradient(135deg, hsl(142 76% 36%) 0%, hsl(142 76% 56%) 100%)' 
              : 'linear-gradient(135deg, hsl(0 84% 60%) 0%, hsl(0 84% 70%) 100%)'};
            border: 4px solid white;
            border-radius: 50% 50% 50% 0;
            display: flex;
            align-items: center;
            justify-content: center;
            box-shadow: 0 6px 20px rgba(0,0,0,0.35), 0 0 0 2px ${isAvailable ? 'rgba(34, 197, 94, 0.3)' : 'rgba(239, 68, 68, 0.3)'};
          ...
          ">
            <div style="transform: rotate(45deg);">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="white" stroke="white" stroke-width="2">
                ...
                <circle cx="12" cy="10" r="3" fill="rgba(255,255,255,0.8)"></circle>
              </svg>
            </div>
          </div>
        </div>
    ...
    <Card className="relative overflow-hidden">
      <style>{`
        ...
        .home-billboard-marker:hover div:last-child {
          ...
          box-shadow: 0 8px 25px rgba(0,0,0,0.45), 0 0 0 3px rgba(255, 255, 255, 0.8) !important;
        }
        .home-billboard-marker:hover div:first-child {
        ...
        .home-billboard-popup .leaflet-popup-tip {
