...
const MapSearchBar = memo(function MapSearchBar({
  ...
  onSelectBillboard,
  onNavigateToCoords,
  placeholder = 'ابحث عن لوحة، منطقة، معلم، مكان...',
  className = ''
}: MapSearchBarProps) {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'billboard': return <MapPin className="w-4 h-4 text-amber-500" />;
  ...
  return (
    <div ref={containerRef} className={`relative ${className}`}>
      <div className="flex items-center gap-2">
        {onRequestLocation && (
          <Button
            ...
            className="w-10 h-10 rounded-xl bg-slate-950/80 hover:bg-slate-900 border border-amber-500/20 text-slate-300 hover:text-amber-500 hover:shadow-[0_0_12px_rgba(245,158,11,0.2)] transition-all shadow-lg backdrop-blur-md"
            title="موقعي الحالي"
          >
        ...
        <div className={`relative flex-1 transition-all duration-300 ${isFocused ? 'scale-[1.01]' : ''}`}>
          <div className={`flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-950/80 backdrop-blur-md border transition-all shadow-lg ${
            isFocused ? 'border-amber-500 shadow-[0_0_15px_rgba(245,158,11,0.15)]' : 'border-amber-500/20'
          }`}>
            ...
            <Input
              ...
              className="border-0 bg-transparent p-0 h-auto text-slate-200 placeholder:text-slate-400/50 focus-visible:ring-0 focus-visible:ring-offset-0 text-right font-bold text-xs"
              dir="rtl"
            />
          </div>
        </div>
      ...
      {showDropdown && suggestions.length > 0 && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-slate-950/95 backdrop-blur-xl border border-amber-500/20 rounded-2xl shadow-2xl z-[2000] max-h-80 overflow-y-auto custom-scrollbar animate-in fade-in slide-in-from-top-1 duration-200">
          {suggestions.map((s, i) => (
            <button
              key={`${s.type}-${i}`}
              className="w-full flex items-center gap-3 px-4 py-3 hover:bg-amber-500/10 hover:text-amber-400 transition-colors text-right border-b border-white/5 last:border-0"
              onMouseDown={(e) => {
                e.preventDefault();
                handleSelect(s);
              }}
            >
              ...
              <div className="flex-1 min-w-0 text-right">
                <p className="text-xs font-bold text-slate-200 truncate">{s.label}</p>
                {s.sublabel && (
                  <p className="text-[10px] text-slate-400 font-medium truncate mt-0.5">{s.sublabel}</p>
                )}
              </div>
              <span className="text-[9px] font-extrabold text-amber-500/80 bg-amber-500/5 px-2 py-0.5 rounded-md flex-shrink-0">
                {getTypeLabel(s.type)}
              </span>
            </button>
          ))}
