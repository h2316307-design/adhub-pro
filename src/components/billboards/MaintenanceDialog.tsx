            ...
            <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg border">
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  try {
                    const updatePayload: any = { is_visible_in_available: newValue };

                    if (newValue) {
                      const statusValue = String((selectedBillboard as any).Status ?? (selectedBillboard as any).status ?? '').trim().toLowerCase();
                      const hasContract = !!((selectedBillboard as any).Contract_Number ?? (selectedBillboard as any).contractNumber);
                      const maintenanceStatus = String((selectedBillboard as any).maintenance_status ?? '').trim().toLowerCase();
                      const maintenanceType = String((selectedBillboard as any).maintenance_type ?? '').trim();
                      ...
                      if (
                        !hasContract &&
                        (statusValue === 'متاح' || statusValue === 'available' || statusValue === '') &&
                        (blockingMaintenanceValues.includes(maintenanceStatus) || blockingMaintenanceTypes.includes(maintenanceType))
                      ) {
                    ...
                    setSelectedBillboard({
                      ...selectedBillboard,
                      is_visible_in_available: newValue,
                      ...(updatePayload.maintenance_status ? { maintenance_status: updatePayload.maintenance_status, maintenance_type: null } : {})
                    });
