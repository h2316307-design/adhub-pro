...
export async function calculateContractDelay(contractNumber: number): Promise<ContractDelayResult | null> {
  try {
    ...
    // جلب مهام التركيب عبر composite_tasks
    ...
    // فلترة اللوحات التي تخص هذا العقد باستخدام billboard_ids من العقد
    ...
    // 6. حساب التأخير لكل لوحة (بما في ذلك اللوحات التي لم تُركّب بعد)
    const details: BillboardDelayInfo[] = [];
    const today = new Date();

    for (const item of uniqueItems) {
      if (!item.selected_design_id) continue;
      ...
      // إذا تم التركيب، نحسب التأخير الفعلي. إذا لم يُركّب بعد، نحسب التأخير حتى اليوم
    ...
    // 7. حساب التأخير من بداية العقد (كم يوم من بداية العقد حتى التركيب الفعلي أو اليوم)
    const fromStartDetails: BillboardContractStartDelay[] = [];
    for (const item of uniqueItems) {
      const compareDate = item.installation_date ? new Date(item.installation_date) : today;
      const daysFromStart = Math.max(0, Math.ceil((compareDate.getTime() - contractStart.getTime()) / (1000 * 60 * 60 * 24)));
      const bb = item.billboard as any;
      const billboardCost = billboardCosts[item.billboard_id] || Number(bb?.Price || 0);
      const dailyValue = contractDurationDays > 0 ? billboardCost / contractDurationDays : 0;
    ...
    return {
      contractNumber,
      totalBillboards: uniqueItems.length,
      delayedBillboards: delayedDetails.length,
      onTimeBillboards: uniqueItems.length - delayedDetails.length,
